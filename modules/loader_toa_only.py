...
class locDL(Dataset):
    def __init__(self,maps_inds=np.zeros(1), phase="train",
        ...
        self.dir_dataset = '/'.join(self.dir_dataset.split('/')[:-2])+ '/png/'
        ...
        self.dir_toa_true = 'dataset/RadioToAImage/True'
        self.dir_toa_est = 'dataset/RadioToAImage/Est'
    ...
    def __getitem__(self, idx):
        ...
        # mat = np.load('/home/hu/uwlc/LocUNet/lib/my_fileCorr.npy',allow_pickle='TRUE').item()
        mat = np.load('dataset/RadioLocSeer/mat/TxRx.npy',allow_pickle=True).item()
        rxx = mat['rxx']
        rxy = mat['rxy']
        ...
        for m in range(self.numTx):
            ...
            image_name_toatrue = os.path.join(self.dir_toa_true, name2)
            image_toatrue = np.asarray(io.imread(image_name_toatrue)) / 255.
            toatrue = image_toatrue[RXr,RXc]
            imgToAtrue = toatrue*np.ones(np.shape(image_toatrue))
            inputEstMaps.append(imgToAtrue)
            image_name_toaest = os.path.join(self.dir_toa_est, name2)
            image_toaest = np.asarray(io.imread(image_name_toaest)) / 255.
            inputEstMaps.append(image_toaest)
